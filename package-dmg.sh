...
LATEST_DMG_PATH="$DIST_DIR/$APP_NAME-latest-Apple-Silicon.dmg"
LATEST_SHA256_PATH="$LATEST_DMG_PATH.sha256"
...
rm -rf "$WORK_DIR"
rm -f "$DMG_PATH" "$DMG_PATH.sha256" "$LATEST_DMG_PATH" "$LATEST_SHA256_PATH"
...
/bin/cp -X "$DMG_PATH" "$LATEST_DMG_PATH"
shasum -a 256 "$LATEST_DMG_PATH" > "$LATEST_SHA256_PATH"
rm -rf "$WORK_DIR"

echo ""
echo "Created: $DMG_PATH"
echo "Checksum: $DMG_PATH.sha256"
echo "Stable release asset: $LATEST_DMG_PATH"
echo "Stable checksum: $LATEST_SHA256_PATH"
